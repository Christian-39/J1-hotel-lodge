...
const API = (() => {
  ...
  function sendReceipt(lookup, opts = {}) {
    const base = BOOKINGS_EP();
    if (!base) throw notConfigured("bookings");
    // The HTTP path renders metadata and performs only a bounded broker publish;
    // PDF generation and SMTP happen on the worker. Broker connect/publish
    // limits on the backend are below this 20-second client deadline, so an
    // outage returns a precise FAILED/BROKER response instead of hanging until
    // Gunicorn's 30-second kill. Callers may still override opts.timeout.
    return post(`${base}/${encodeURIComponent(lookup)}/send-receipt/`, {}, { timeout: 20000, ...opts });
  }
