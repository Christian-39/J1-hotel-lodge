...
/* ==========================================================================
   ...
   Every icon is usable either as:
     - <span data-icon="name" data-size="20"></span> (auto-injected by UI.init)
     - or via JONE.icons.get("name")
   No emojis. No mixed icon libraries.
   ========================================================================== */
...
(function () {
  ...
  const PATHS = {
    ...
    ruler: '<path d="M21.3 8.7l-8.6 8.6a2 2 0 0 1-2.8 0l-6.2-6.2a2 2 0 0 1 0-2.8l8.6-8.6a2 2 0 0 1 2.8 0l6.2 6.2a2 2 0 0 1 0 2.8z"/><path d="M8 8l2 2M11 5l2 2M5 11l2 2M14 2l2 2M2 14l2 2"/>',
    wifi: '<path d="M5 12.55a11 11 0 0 1 14.08 0M1.42 9a16 16 0 0 1 21.16 0M8.53 16.11a6 6 0 0 1 6.95 0M12 20h.01"/>',
    coffee: '<path d="M18 8h1a4 4 0 0 1 0 8h-1M2 8h16v9a4 4 0 0 1-4 4H6a4 4 0 0 1-4-4V8z"/><line x1="6" y1="1" x2="6" y2="4"/><line x1="10" y1="1" x2="10" y2="4"/><line x1="14" y1="1" x2="14" y2="4"/>',
    ...
    /* Refund / undo — used by the refunds table and the missed-bookings page. */
    rotateCcw: '<polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/>',
    /* A guest who did not arrive (no-show action). */
  ...
  // Auto-inject every [data-icon] element.
  function inject(root = document) {
    root.querySelectorAll("[data-icon]").forEach((node) => {
      const name = node.getAttribute("data-icon");
      const size = parseInt(node.getAttribute("data-size") || "20", 10);
      const svg = get(name).replace('width="24" height="24"', `width="${size}" height="${size}"`);
      node.innerHTML = svg;
      node.setAttribute("role", "img");
      if (!node.getAttribute("aria-label")) node.setAttribute("aria-hidden", "true");
      // Preserve explicit aria-label on icon-only buttons
      if (node.closest("button") && node.closest("button").getAttribute("aria-label")) {
